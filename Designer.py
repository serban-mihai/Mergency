        ...
        BoxLayout:
            ...
            BoxLayout:
                ...
                MDFillRoundFlatButton:
                    ...
                    pos_hint: {'center_x': .5, 'center_y': .25}
                    width: dp(200)
                    on_release: app.button_connect(host.text, service.text, port.text, user.text, password.text)
                ...
                MDRoundFlatButton:
                    ...
                    pos_hint: {'center_x': .5, 'center_y': .25}
                    width: dp(200)
                    on_release: app.button_disconnect()
...
class Designer(object):
    ...
    def show_manager(self):
        if not self.manager:
            ...
            self.manager = Factory.Manager()
            self.main_widget.ids.scr_mngr.add_widget(self.manager)
    ...
    def show_database(self):
        if not self.database:
            Builder.load_string(database)
            self.database = Factory.Database()
            self.main_widget.ids.scr_mngr.add_widget(self.database)
            self.database.ids.host.bind(on_release=self.button_connect)
            self.database.ids.service.bind(on_release=self.button_connect)
            self.database.ids.port.bind(on_release=self.button_connect)
            self.database.ids.user.bind(on_release=self.button_connect)
            self.database.ids.password.bind(on_release=self.button_connect)
            # self.database.ids.text_field_error.bind(
            #     on_text_validate=self.set_error_message,
            #     on_focus=self.set_error_message)
        self.main_widget.ids.scr_mngr.current = 'database'
